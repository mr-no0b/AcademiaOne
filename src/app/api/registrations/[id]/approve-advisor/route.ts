...
async function approveByAdvisorHandler(req: AuthenticatedRequest, context: { params: { id: string } }) {
  const user = req.user!;
  const registrationId = context.params.id;

  const registration = await registrationFacade.approveByAdvisor(registrationId, user.userId);
